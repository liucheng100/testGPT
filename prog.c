...
int checksum(package) struct pkt package;
{
  int sum = 0;
  sum = sum + package.seqnum;
  sum += package.acknum;
  for(int i=0;i<20;i++){
...
A_output(message)
  struct msg message;
{
  // 将message封装成package，并发送给layer3(tolayer3)
  struct pkt package;
  package.seqnum = S_A.seq;
  memmove(package.payload, message.data, 20);
  package.checksum = checksum(package);
  printf("adsf3");
  tolayer3(0,package);
  S_A.last_pkt = package;
  starttimer(0, S_A.exRTT);
}
  ...
  struct pkt packet;
{
  ...
  if(packet.checksum != checksum(packet) ){
    printf("ACK的checksum有问题位翻转\n");
    return;
  }
  if( packet.acknum != S_A.seq ){
    printf("ACK不符合对应发送方的seq\n");
    return;
  }
  //成功收到ack，seq下一个，停止计时器
  printf("成功校验ack即将进行下一步发包\n");
  S_A.seq = 1 - S_A.seq;
  stoptimer(0);
}
...
{
  // 重发最新包
  tolayer3(0, S_A.last_pkt);
  // stoptimer(0);
  starttimer(0, S_A.exRTT); //我不太清楚为啥这个没有先stop
...
B_input(packet)
  struct pkt packet;
{
  printf("aisdufhaisudhf\n");
  if(packet.checksum != checksum(packet)){
  printf("!22221\n");
    sendACK(1 - R_A.seq);
    return;
  }
  // 这里可能因之前发送的ack丢失而收到发送方发来的重复旧包
  if(packet.seqnum != R_A.seq){
  printf("!433311111\n");
      sendACK(1 - R_A.seq);
    return;
  }
   ...
   while (1) {
        ...
        if (eventptr->evtype == FROM_LAYER5 ) {
            ...
            for (i=0; i<20; i++)  
              ...
              printf("111111111111TIMER_INTERRUPT\n");
            if (eventptr->eventity == A) 
	            A_timerinterrupt();
             else
	            B_timerinterrupt();
...
init()                         /* initialize the simulator */
{
  ...
  float jimsrand();
   ...
   printf("-----  Stop and Wait Network Simulator Version 1.111 -------- \n\n");
